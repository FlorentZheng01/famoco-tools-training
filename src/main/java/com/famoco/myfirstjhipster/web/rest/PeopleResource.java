...
/**
 * REST controller for managing {@link com.famoco.myfirstjhipster.domain.People}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PeopleResource {
    ...
    public List<People> getAllPeople(@RequestParam(required = false) String filter) {
        if ("country-is-null".equals(filter)) {
            log.debug("REST request to get all Peoples where country is null");
            return StreamSupport
                .stream(peopleRepository.findAll().spliterator(), false)
                .filter(people -> people.getCountry() == null)
                .collect(Collectors.toList());
        }

        if ("odf-is-null".equals(filter)) {
            log.debug("REST request to get all Peoples where oDF is null");
            return StreamSupport
                .stream(peopleRepository.findAll().spliterator(), false)
                .filter(people -> people.getODF() == null)
                .collect(Collectors.toList());
        }
